...
def pivot_finder(set_list:List[Set[int]],piv_list:List[Set[int]]) -> List[Set[int]]:
    ...
    if not len(set_list_update):
        return piv_list
    
    else:
        ord_list = order_set_list(set_list_update)
        new_list = []
        ...
        if len(ord_list)==1:
            ...
            for chk in ord_list:
                if q in chk:
                    new_list.append(chk^new_pivot)
                else:
                    new_list.append(chk)
            piv_list.append(new_pivot)

            return pivot_finder(new_list,piv_list)
...
def image_checker(set_list:List[Set[int]],elem_set:Set[int]) -> bool:
    ...
    # else:
    #     pivot_rows = pivot_finder(set_list,[])
    #     new_rows = pivot_finder(pivot_rows+[elem_set],[])
    #     if len(new_rows) == len(pivot_rows):
    #         return True
    #     else:
    #         return False
