...
from qulacs.gate import CNOT,H,RY,S,Sdag
from qulacs.gate import X,Y,Z,P0
from qulacs.gate import DepolarizingNoise, TwoQubitDepolarizingNoise

def controlled_hadamard(state:QuantumState,control_block:list[int],target_block:list[int]) -> QuantumState:
...
def controlled_y(state:QuantumState,control_block:list[int],target_block:list[int]) -> QuantumState:
...
def magic_state_initialization(state:QuantumState,qubit:int,angle = np.pi/4.0) -> QuantumState:
    r"""
...
def repetition_encoder(state:QuantumState,block:list[int],flag = False) -> QuantumState:
    r"""
...
def noisy_controlled_hadamard(state:QuantumState,control_block:list[int],target_block:list[int],perr:float) -> QuantumState:
...
def noisy_controlled_y(state:QuantumState,control_block:list[int],target_block:list[int],perr:float) -> QuantumState:
...
def noisy_magic_state_initialization(state:QuantumState,qubit:int,perr:float, angle = np.pi/4.0) -> QuantumState:
...
def noisy_repetition_encoder(state:QuantumState,block:list[int],perr:float,flag = False) -> QuantumState:
    r"""
    Encode the input state, assumed to be on the qubit at block[0], into the
    the repetition code supported on the qubits in the list block.
    ...
    TwoQubitDepolarizingNoise(first_half[0],second_half[0],perr).update_quantum_state(state)
    ...
    if block_length%2:
        CNOT(second_half[-2],second_half[-1]).update_quantum_state(state)
        TwoQubitDepolarizingNoise(second_half[-2],second_half[-1],perr).update_quantum_state(state)
    ...
    if flag:
        CNOT(first_half[-1],second_half[-1]).update_quantum_state(state)
        TwoQubitDepolarizingNoise(first_half[-1],second_half[-1],perr).update_quantum_state(state)
...
def noisy_steane_encoder(state:QuantumState,block:list[int],perr:float) -> QuantumState:
    r"""
    Encode the input state, assumed to be on the qubit at block[0], into the
    the Steane code supported on the seven qubits in the list block.
